...
public class UTestNodeGeneConfiguration_XML extends AbstractXmlFormatTest<NodeGeneConfiguration> {
	private static final Object[][]	INSTANCES;
	static {
		INSTANCES = TestUtils.createData(NodeGeneConfiguration.builder(XId.newId("UTestNodeGeneConfiguration_XML")).newInstance());
	}

	public UTestNodeGeneConfiguration_XML() {
		super(NodeGeneConfiguration.XML_BINDING);
	}

	@Override
	protected String generateExpectedXMLString(NodeGeneConfiguration t) {
		StringBuilder sb = new StringBuilder().append(XML_HEADER);
		sb.append(String.format("<%s id=\"%s\">\n", NodeGeneConfiguration.class.getSimpleName(), t.getId()));
		...
		sb.append(String.format("</%s>", NodeGeneConfiguration.class.getSimpleName()));
		return sb.toString();
	}
