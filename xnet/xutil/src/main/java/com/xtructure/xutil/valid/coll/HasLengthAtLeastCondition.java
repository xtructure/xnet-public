 ...
 * has at least a given length.
 * 
 * @author Luis Guimbarda
 * 
 */
public class HasLengthAtLeastCondition extends AbstractValueCondition<Integer> {
	/**
	 * Creates a new {@link HasSizeCondition} that is satisfied by strings or
	 * arrays with at least the given size.
	 * 
	 * @param length
	 *            the minimum length of strings or arrays that satisfy this
	 *            condition
	 */
	public HasLengthAtLeastCondition(Integer length) throws IllegalArgumentException {
		super(length, false);
		if (length < 0) {
	 ...
	 * {@inheritDoc}
	 * 
	 * @return true if the given object is string or array and has length at
	...
	public String toString() {
		return String.format("must have length at least %d", getValue());
	}
}
