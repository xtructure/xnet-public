...
public class UTestHasSizeCondition {
	...
	public void constructorSucceeds() {
		if (new HasSizeCondition(RandomUtil.nextInteger(10)) == null) {
	...
	public void isSatisfiedByReturnsExpectedBoolean(Object object) {
		HasSizeCondition predicate = new HasSizeCondition(0);
		if (BAD.contains(object) || ONE_SIZE.contains(object) || TWO_SIZE.contains(object)) {
		...
		predicate = new HasSizeCondition(1);
		if (BAD.contains(object) || ZERO_SIZE.contains(object) || TWO_SIZE.contains(object)) {
			if (predicate.isSatisfiedBy(object)) {
				throw new AssertionError();
			}
		} else {
		...
		predicate = new HasSizeCondition(2);
		...
		predicate = new HasSizeCondition(3);
		if (predicate.isSatisfiedBy(object)) {
			throw new AssertionError();
		}
	}
	...
	public void toStringReturnsExpectedString() {
		...
		if (!String.format("must have size %d", size).equals(new HasSizeCondition(size).toString())) {
			throw new AssertionError();
		}
	}
}
