...
public class UTestAbstractMetaCondition_XML extends AbstractXmlFormatTest<DummyMetaCondition> {
	private static final Object[][]	INSTANCES;
	static {
		INSTANCES = TestUtils.createData(//
				new DummyMetaCondition(isNull()),//
				new DummyMetaCondition(isNotNull(), isNotEmpty(), everyElement(isNotNull())),//
				new DummyMetaCondition(isEmpty())//
				);
	}

	protected UTestAbstractMetaCondition_XML() {
		super(null);
	}

	@Override
	protected String generateExpectedXMLString(DummyMetaCondition t) {
		StringBuilder sb = new StringBuilder().append(XML_HEADER);
		sb.append(String.format("<%s>\n", DummyMetaCondition.class.getName()));
		...
		sb.append(String.format("</%s>", DummyMetaCondition.class.getName()));
		return sb.toString();
	}
