...
public class UTestFileXOption {
	public void constructorSucceeds() {
		assertThat("",//
				new FileXOption("name", "opt", "longOpt", "description"),//
				isNotNull());
	}

	public void processValueReturnsExpectedObject() throws ParseException {
		XOption<?> opt = new FileXOption("name", "opt", "longOpt", "description");
		Options options = new Options();
		options.addOption(opt);
		String[] args = new String[] { "-opt", "filename" };
		XOption.parseArgs(options, args);
		assertThat("",//
				opt.processValue(), and(//
						isNotNull(),//
						isOfExactType(File.class),//
						isEqualTo(new File("filename"))));
	}
}
