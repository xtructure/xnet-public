...
public class NodeTracer extends AbstractStandardXComponent {
	 ...
	 */
	public static NodeTracer getInstance(XId id, AbstractStandardXSimulation sim, Node node, List<Float> expectedValues, Float epsilon) {
		return new NodeTracer(id, sim, Collections.singleton(node.getId()), expectedValues, epsilon);
	}
	 ...
	 * @param epsilon the epsilon
	 */
	private NodeTracer(XId id, AbstractStandardXSimulation sim, Set<XId> targetIds, List<Float> expectedValues, Float epsilon) {
...
//			System.out.println(String.format("Tracer %s => Checking %s at time %s: expected(%f) seen(%f)", getId(), sourceAddress.getPartId(), SIM.getTime(), backEnergy, expected));
			validateState(sourceAddress.getPartId().toString(), backEnergy, isGreaterThanOrEqualTo(expected - EPSILON), isLessThanOrEqualTo(expected + EPSILON));
		}
	}
